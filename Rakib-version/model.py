...
class SimpleCNN_shallow(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim=256, num_ftrs=84):
        super(SimpleCNN_shallow, self).__init__()
        self.nclasses = 10
        self.conv1 = nn.Conv2d(3, 6, 5)
        self.relu = nn.ReLU()
        self.pool = nn.MaxPool2d(2, 2)
        ...
        # last layer
        self.l3 = nn.Linear(output_dim, self.nclasses)

        #shallow block 1
        self.s1_fc1 = nn.Linear(6*14*14, 256)
        self.s1_fc2 = nn.Linear(256, self.nclasses)

        #shallow block 2
        self.s2_fc1 = nn.Linear(16*5*5, 256)
        self.s2_fc2 = nn.Linear(256, self.nclasses)


    def forward(self, x):

        x = self.pool(self.relu(self.conv1(x)))
        # print("Input size to first shallow block: ", x.shape)
        shallow_rep1 = self.s1_fc1(x.view(-1, 6*14*14))
        # print("Representation size of the first shallow block: ", shallow_rep1.shape)
        shallow_output1 = self.s1_fc2(shallow_rep1)
        # print("Output size of first shallow block: ", shallow_output1.shape)

        x = self.pool(self.relu(self.conv2(x)))
        # print("Input size to second shallow block: ", x.shape)
        shallow_rep2 = self.s2_fc1(x.view(-1, 16*5*5))
        # print("Representation size of the second shallow block: ", shallow_rep2.shape)
        shallow_output2 = self.s2_fc2(shallow_rep2)
        # print("Output size of first shallow block: ", shallow_output2.shape)
        ...
        x = F.relu(x)
        x = self.l2(x)
        # print("Representation size of the projection head: ", x.shape)

        y = self.l3(x)
        # print("Output size of the model: ", y.shape)
        return shallow_rep1, shallow_output1, shallow_rep2, shallow_output2, x, y

class SimpleCNN_MNIST_shallow(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim=256, num_ftrs=84):
        super(SimpleCNN_MNIST_shallow, self).__init__()
        self.conv1 = nn.Conv2d(1, 6, 5)
        self.relu = nn.ReLU()
        self.pool = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(6, 16, 5)
        ...
        # last layer
        self.l3 = nn.Linear(output_dim, 10)

        #shallow block 1
        self.s1_fc1 = nn.Linear(6*12*12, 256)
        self.s1_fc2 = nn.Linear(256, 10)

        #shallow block 2
        self.s2_fc1 = nn.Linear(16*4*4, 256)
        self.s2_fc2 = nn.Linear(256, 10)
    ...
    def forward(self, x):
        ...
        x = self.pool(self.relu(self.conv1(x)))
        # print("Input size to first shallow block: ", x.shape)
        shallow_rep1 = self.s1_fc1(x.view(-1, 6*12*12))
        # print("Representation size of the first shallow block: ", shallow_rep1.shape)
        shallow_output1 = self.s1_fc2(shallow_rep1)
        # print("Output size of first shallow block: ", shallow_output1.shape)
        ...
        x = self.pool(self.relu(self.conv2(x)))
        # print("Input size to second shallow block: ", x.shape)
        shallow_rep2 = self.s2_fc1(x.view(-1, 16*4*4))
        # print("Representation size of the second shallow block: ", shallow_rep2.shape)
        shallow_output2 = self.s2_fc2(shallow_rep2)
        # print("Output size of second shallow block: ", shallow_output2.shape)
